...
def render(df_clean, cfg, sel_group):
    ...
    df_m['income_label'] = df_m[ri_col].fillna('Chưa rõ') if ri_col in df_m.columns else df_m.get('income_group', pd.Series('Chưa rõ')).astype(str)
    ...
    df_tree = df_m[tree_mask].groupby(['intent_clean', 'income_label', 'phat_label', 'tenure_label']
    ).agg(Count=('EI','count'), EI_avg=('EI','mean')).reset_index()
    df_tree['EI_avg'] = df_tree['EI_avg'].round(1)
    df_tree = df_tree[df_tree['Count'] >= 3]

    if len(df_tree) > 5:
        fig = px.treemap(df_tree, path=['intent_clean','income_label','phat_label','tenure_label'],
    ...
    if len(df_risk) > 5 and len(df_ok) > 5:
        ...
        phat_risk = df_risk.get('tong_phat', pd.Series(0)).mean()
        phat_ok = df_ok.get('tong_phat', pd.Series(0)).mean()
        ...
        don_risk = df_risk[don_col].mean() if don_col else 0
        don_ok = df_ok[don_col].mean() if don_col else 0
        ...
        comp_df = pd.DataFrame({
            'Chỉ số': ['Thu nhập (tr)', 'Phạt (tr)', 'Năng suất Giao'],
            'Muốn nghỉ': [income_risk, phat_risk, don_risk],
            'Gắn bó': [income_ok, phat_ok, don_ok],
        })
