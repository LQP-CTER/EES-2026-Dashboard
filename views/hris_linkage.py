...
""")
    ...
    if len(df_low) < 10 or len(df_high) < 10:
        ...
        # 2. Quản lý phân không công bằng (C4 + C16)
        for q, q_name in [('C4', 'Phân đơn công bằng'), ('C16', 'Phân đơn khu vực hợp lý')]:
            if q in df.columns:
        ...
        with col_box:
            # Box plot: C4 (Phân đơn công bằng) theo nhóm NS
            if 'C4' in df.columns:
                fig_box = go.Figure()
                for lbl in ns_label_order:
                    grp_data = df[df['_ns_label'] == lbl]['C4'].dropna()
                    if len(grp_data) > 5:
